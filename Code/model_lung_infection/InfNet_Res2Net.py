...
class Inf_Net(nn.Module):
    ...
    def forward(self, x):   #(24,3,256,256)
        ...
        # ---- low-level features ----
        x = self.resnet.maxpool(x)      # bs, 64, 88, 88    #(24,64,64,64)
        x1 = self.resnet.layer1(x)      # bs, 256, 88, 88   #(24,256,64,64)

        # ---- high-level features ----
        x2 = self.resnet.layer2(x1)     # bs, 512, 44, 44   #(24,512,32,32)
        x3 = self.resnet.layer3(x2)     # bs, 1024, 22, 22  #(24,1024,16,16)
        x4 = self.resnet.layer4(x3)     # bs, 2048, 11, 11  #(24,2048,8,8)
        x2_rfb = self.rfb2_1(x2)        # channel -> 32     #(24,32,32,32)
        x3_rfb = self.rfb3_1(x3)        # channel -> 32     #(24,32,16,16)
        x4_rfb = self.rfb4_1(x4)        # channel -> 32     #(24,32,8,8)
        ...
        x = -1*(torch.sigmoid(crop_4)) + 1  # reverse   #(24,1,8,8)
        x = x.expand(-1, 2048, -1, -1).mul(x4)      #(24,2048,8,8)
        x = torch.cat((self.ra4_conv1(x), F.interpolate(edge_guidance, scale_factor=1/8, mode='bilinear')), dim=1)  #(24,320,8,8)
        x = F.relu(self.ra4_conv2(x))   #(24,256,8,8)
        x = F.relu(self.ra4_conv3(x))   #(24,256,8,8)
        x = F.relu(self.ra4_conv4(x))   #(24,256,8,8)
        ra4_feat = self.ra4_conv5(x)    #(24,1,8,8)
        x = ra4_feat + crop_4   # element-wise addition    #(24,256,8,8)
        lateral_map_4 = F.interpolate(x,
                                      scale_factor=32,
                                      mode='bilinear')  # NOTES: Sup-2 (bs, 1, 11, 11) -> (bs, 1, 352, 352)    #(24,1,256,256)

        # ---- reverse attention branch_3 ----
        crop_3 = F.interpolate(x, scale_factor=2, mode='bilinear')         #(24,1,16,16)
        x = -1*(torch.sigmoid(crop_3)) + 1          #(24,1,16,16)
        x = x.expand(-1, 1024, -1, -1).mul(x3)      #(24,1024,16,16)
        x = torch.cat((self.ra3_conv1(x), F.interpolate(edge_guidance, scale_factor=1/4, mode='bilinear')), dim=1)#(24,128,16,16)
        x = F.relu(self.ra3_conv2(x))       #(24,64,16,16)
        x = F.relu(self.ra3_conv3(x))       #(24,64,16,16)
        ra3_feat = self.ra3_conv4(x)        #(24,1,16,16)
        x = ra3_feat + crop_3               #(24,1,16,16)
        lateral_map_3 = F.interpolate(x,
                                      scale_factor=16,
                                      mode='bilinear')  # NOTES: Sup-3 (bs, 1, 22, 22) -> (bs, 1, 352, 352) #(24,1,256,256)

        # ---- reverse attention branch_2 ----
        crop_2 = F.interpolate(x, scale_factor=2, mode='bilinear')  #(24,1,32,32)

        x = -1*(torch.sigmoid(crop_2)) + 1       #(24,1,32,32)
        x = x.expand(-1, 512, -1, -1).mul(x2)     #(24,512,32,32)
        x = torch.cat((self.ra2_conv1(x), F.interpolate(edge_guidance, scale_factor=1/2, mode='bilinear')), dim=1) #(24,128,32,32
        x = F.relu(self.ra2_conv2(x))        #(24,64,32,32)
        x = F.relu(self.ra2_conv3(x))       #(24,64,32,32)
        ra2_feat = self.ra2_conv4(x)        #(24,1,32,32)
        x = ra2_feat + crop_2               #(24,1,32,32)
